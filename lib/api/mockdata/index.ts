...
const freeImageUrls = [
  ...
  ...new Array(100)
    .fill(0)
    .map((e, i) => `https://picsum.photos/200/200?random=${i + 1}`),
];
...
export const mockUserIndustry: UserIndustry[] = Array.from(
  { length: 3 },
  (_, index) => ({
...
export const mockProfileInfo: (u: UserType, i?: number) => ProfileInfo = (
  userType: UserType,
...
export const mockCampaign: (index?: number) => Campaign = (
  index = Math.ceil(Math.random() * 10)
) => ({
  ...
  creative: {
    paymentType: "Fixed",
    startDate: new Date(
      `2023-06-${index < 10 ? "0" + index : index}T00:00:00Z`
    ),
    endDate: new Date(
      `2023-07-${index < 10 ? "0" + index : index}T00:00:00Z`
...
export const generateRandomIndustries = (count: number): Industry[] => {
  ...
  const selectedNames = shuffled.slice(
    0,
    Math.min(count, industryNames.length)
  );
...
export function generateMockInfluencerBookmarks(
  count: number
): InfluencerBookmark[] {
    ...
    const mockInfluencerBookmark: InfluencerBookmark = {
      ...
      campaignRecord: {
        ...
        creative: {
          ...
          creativeBrief:
            "Create engaging content showcasing our new product line",
...
export function generateMockAddress(): Address {
  ...
  return {
    street: `${Math.floor(Math.random() * 1000) + 1} ${
...
export function generateMockBankDetails(): BankDetails {
  ...
  return {
    ...
    bankAddress: `${Math.floor(Math.random() * 1000) + 1} ${
      ["Main St", "Broadway", "Park Ave", "Wall St", "5th Ave"][
    ...
    accountName: `${getRandomFirstName()} ${getRandomMiddleName()} ${
...
export function generateMockAgeDemographicsData(
  num: number
): AgeDemographicsData[] {
  const ageGroups = [
    "13-17",
...
export function generateMockSearchInfluencer(num: number): SearchInfluencer[] {
  ...
  for (let i = 0; i < num; i++) {
    mockInfluencers.push({
      ...
      biography:
        Math.random() > 0.5
          ? `This is a mock biography for Influencer ${i + 1}.`
          : undefined,
...
export function generateMockApplicationsData(num: number): Application[] {
  ...
  for (let i = 0; i < num; i++) {
    mockApplications.push({
      ...
      referenceLink:
        Math.random() > 0.5
          ? `https://example.com/reference-${i + 1}`
          : undefined,
      email: `influencer${i + 1}@example.com`,
      phoneNumber: `+1${Math.floor(1000000000 + Math.random() * 9000000000)}`,
      profilePicture: getRandomImage(),
...
export function generateMockApprovedInfluencersData(
  num: number
): ApprovedCampaignInfluencer[] {
  ...
  for (let i = 0; i < num; i++) {
    mockApprovedInfluencers.push({
      ...
      socialMediaPlatforms: JSON.stringify(
...
export function generateMockCampaignDistribution(): CampaignDistribution {
  return {
    audienceDistributionSummary: {
      topCountry: ["USA", "UK", "Canada", "Australia"][
        Math.floor(Math.random() * 4)
      ],
      topCity: ["New York", "London", "Toronto", "Sydney"][
        Math.floor(Math.random() * 4)
      ],
      topGender: Math.random() > 0.5 ? "Male" : "Female",
      topAge: ["18-24", "25-34", "35-44", "45-54"][
        Math.floor(Math.random() * 4)
      ],
    },
...
export function generateMockSocialMediaInsight(): SocialMediaInsight {
  return {
    ...
    biography: `This is a mock biography for ${Math.random()
      .toString(36)
      .substr(2, 6)}`,
...
export function generateMockFullCampaign(): FullCampaign {
  return {
    ...
    overview: {
      ...
      briefDescription: `This is a mock campaign description for ${Math.random()
        .toString(36)
        .substr(2, 6)}`,
      website: `https://www.${Math.random().toString(36).substr(2, 8)}.com`,
    ...
    influencer: {
      influencerCategory: ["Fashion", "Beauty", "Lifestyle", "Travel", "Food"]
        .sort(() => 0.5 - Math.random())
        .slice(0, Math.floor(Math.random() * 3) + 1),
      audienceSize: ["1K-10K", "10K-50K", "50K-100K", "100K+"]
        .sort(() => 0.5 - Math.random())
        .slice(0, Math.floor(Math.random() * 2) + 1),
      audienceGender: ["Male", "Female", "Other"]
        .sort(() => 0.5 - Math.random())
        .slice(0, Math.floor(Math.random() * 3) + 1),
      audienceAgeGroup: ["13-17", "18-24", "25-34", "35-44", "45+"]
        .sort(() => 0.5 - Math.random())
        .slice(0, Math.floor(Math.random() * 3) + 1),
      audienceLocation: ["USA", "UK", "Canada", "Australia", "Other"]
        .sort(() => 0.5 - Math.random())
        .slice(0, Math.floor(Math.random() * 3) + 1),
