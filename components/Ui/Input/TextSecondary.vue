<template>
    <div class="inline-block p-0 m-0 border-none w-full outline-none bg-transparent relative">
        <div class="p-0 m-0 border-none w-full outline-none bg-transparent relative">
            <input ref="input" v-bind="$attrs" @input="update" :value="props.modelValue" :class="{
                '!border-red-600': props.errorMessage,
                'input-search': props.search,
                'border border-[#BBBBBB] outline-none shadow-none focus:shadow-none px-[1.5rem] bg-transparent w-full py-[0.75rem] dark:text-black rounded-lg text-base placeholder:text-[#B0B0B0]': true,
            }" />
            ...
            <div v-if="props.passwordToggle" tabindex="1"
                ...
                <UtSvg @click="togglePasswordVisibility" name="eye" :class="[
        ...
        <transition>
            <div v-show="props.errorMessage" class="text-red-600 text-left text-[0.875rem]">
                {{ props.errorMessage }}
            </div>
        </transition>
    </div>
