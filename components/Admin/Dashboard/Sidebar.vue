<template>
    <div class="h-full w-full relative">
        <div class="flex flex-col">
            <button class="absolute block md:hidden top-[20px] right-[20px]" @click="$emit('close-sidebar')">
            ...
            <div class="pt-[1.75rem] px-[1.125rem]">
                ...
                <div class="mt-[4.625rem]">
                    ...
                    <AdminDashboardNavigatorMenuItem label="Darkmode" icon="nav/darkmode">
                        <template #right>
                            <UiInputSwitch v-model="mode" true-value="light" false-value="dark" />
                        </template>
        ...
        <div>
            <div class="px-[1.125rem] dark:border-slate-900">
                <div class="mt-[1rem] mb-[3.75rem] flex flex-col gap-[1rem]">
                    <AdminDashboardNavigatorMenuItem label="Settings" icon="nav/setting" to="Settings" />
                    <AdminDashboardNavigatorMenuItem @click="logout" class="text-red-600" label="Log Out"
                        :loading="loading" icon="nav/logout" />
                </div>
            </div>
        </div>
    </div>
