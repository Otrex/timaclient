<template>
  <div>
    ...
    >
      <div>
        <label class="block mb-[0.875rem]">Campaign Name</label>
        <UiInputText
          class="w-full"
    ...
    <div class="mt-3">
      ...
      <template v-else>
        ...
        >
          <template v-for="campaign in search" :key="campaign.campaign_id">
            <NuxtLink
              class="w-full"
              :to="{
...
import type { GetSearchCampaignsResponse } from "~/lib/interfaces/response";
...
const search = ref<GetSearchCampaignsResponse["data"]>([]);
...
const { state, execute } = useRequestState({
  action: () =>
    api.influencerSearchCampaigns({
      campaignName: form.campaignName,
      socialMediaPlatform: form.socialMediaPlatform,
      audienceDemographics: [form.age].filter((e) => e),
      category: [form.category].filter((e) => e),
    }),
  onSuccess: (response) => {
...
const ageGroup = computed(() => optionsStore.campaignOptions?.ageGroup || []);
